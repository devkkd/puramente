...
export default function OurStory() {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setIsVisible(true);
          // Optional: uncomment to only trigger the animation once
  ...
  return (
    <section className="w-full py-16 bg-white font-mona overflow-hidden" ref={sectionRef}>
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row gap-12 lg:gap-16 items-center lg:items-start">
        ...
        <div 
          ...
          `}
        >
          <div className="relative w-full aspect-[4/5] sm:aspect-square lg:aspect-[3/4] bg-white flex items-center justify-center p-4">
            <img 
        ...
        <div className="w-full lg:w-[60%] flex flex-col pt-4 lg:pt-8">
          ...
          >
            {/* Top Subheading */}
            <div className="flex items-center gap-4 text-[#00a3c4] text-base font-normal uppercase mb-6 tracking-wide">
              <span className="w-16 h-px bg-[#00a3c4]"></span>
              <span>Our Story</span>
            </div>

            {/* Main Heading */}
            <h2 className="font-playfair text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-8 leading-tight">
              Rooted in Jaipur, Worn <span className="italic text-[#00a3c4] font-medium">Worldwide</span>
            </h2>

            {/* Paragraphs */}
            <div className="flex flex-col gap-6 text-sm font-normal text-gray-800 leading-relaxed mb-10 max-w-2xl">
              <p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12 max-w-2xl">
            {features.map((feature, index) => {
              // Calculate a staggered delay so they pop up one by one after the text (400ms base + 150ms per card)
              const animationDelay = `${400 + index * 150}ms`;

              return (
                <div 
                  key={index} 
                  className={`border border-[#00a3c4] p-6 flex flex-col gap-3 hover:bg-[#00a3c4]/5 transform transition-all duration-700 ease-out
                    ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}
                  `}
                  style={{ transitionDelay: isVisible ? animationDelay : '0ms' }}
                >
                  <h3 className="text-lg font-bold text-[#00a3c4] tracking-wide">
                    {feature.title}
                  </h3>
          ...
          >
            <Link 
              href="/ourStory" 
              className="inline-flex items-center text-[#00a3c4] text-sm uppercase tracking-widest hover:text-[#0082a4] transition-colors pb-1 border-b border-[#00a3c4]/30 hover:border-[#0082a4]"
            >
