...
import { getAdminContactEnquiries, updateContactEnquiryStatus, deleteAdminContactEnquiry } from "@/lib/api";
import { 
  Mail, 
  ChevronDown, 
  CheckCircle2, 
...
export default function AdminContactPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteAdminContactEnquiry(id);
      if (res.success) {
  ...
  const filteredEnquiries = enquiries.filter(enq => {
    const matchesSearch = 
      ...
      enq.country?.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    if (startDate) {
      ...
      if (enqDate < sDate) matchesDate = false;
    }
    if (endDate) {
      const eDate = new Date(endDate);
      eDate.setHours(23, 59, 59, 999);
      const enqDate = new Date(enq.createdAt);
      if (enqDate > eDate) matchesDate = false;
    }
    ...
    <div className="max-w-7xl mx-auto font-mona">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[40vh] bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">No Enquiries Found</h2>
          <p className="text-gray-500 max-w-md mb-4">No enquiries match your search filters.</p>
          <button
            onClick={() => {
              setSearchQuery("");
              setStartDate("");
                          ...
                          <div className="p-6 md:p-8 animate-in slide-in-from-top-2 fade-in duration-200">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl border border-[#0082A4]/20 mb-6 shadow-sm gap-4">
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="text-sm font-bold text-gray-900 mr-2">Update Status:</span>
