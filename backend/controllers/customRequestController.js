...
exports.submitCustomRequest = async (req, res) => {
  try {
    ...
    // 1. Upload the reference image to Cloudflare if provided
    if (req.file) {
    ...
    // 3. Send Response IMMEDIATELY so the frontend doesn't hang
    res.status(201).json({
      success: true,
    ...
    // 4. Send Email Asynchronously (Decoupled from frontend response)
    try {
      const emailHtml = `
      ...
      console.error("Non-fatal Email Error:", emailError);
      // We don't return res.status(500) here because the DB already saved successfully!
    }

  } catch (error) {
    console.error("Custom Request Error:", error);
    // Only throw 500 if the database save or image upload actually crashes
    if (!res.headersSent) {
